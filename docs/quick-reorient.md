...
BoneWeaver 0.3.0 adds a whole-Armature workflow at the top of the Sidebar:
...
3. Click the one-click action and confirm.
...
The action performs source capture, immutable planning, preflight, mutation,
post-validation, and Snapshot persistence in one operator call. It does not
require the user to run Analyze and Apply separately.
...
digests. Post-validation verifies target values, invariant fields, connected
component geometry, native connectivity, mesh digests, and neutral evaluated
mesh tolerance. Any failure rolls back automatically.
