...
from boneweaver.controllers.selection import SelectionController
from boneweaver.controllers.session import SessionController
from boneweaver.core.runtime_store import has_plan, put_plan
...
class ControllerLifecycleTests(unittest.TestCase):
    ...
    def test_load_and_undo_invalidate_plan_and_preview_without_deleting_snapshot(self):
        ...
        self.assertEqual("BONEWEAVER_SNAPSHOT::1", runtime.snapshot_text_name)
        self.assertFalse(PreviewController.is_enabled())
        self.assertEqual("BONEWEAVER_SCENE_CHANGED_RECHECK", runtime.last_error)
    ...
    def test_workflow_operators_do_not_assign_runtime_fields(self):
        root = Path(__file__).resolve().parents[1] / "boneweaver" / "operators"
        offenders = []
    ...
    def test_algorithm_setting_stales_plan_but_preview_setting_does_not(self):
        ...
        make_bound_mesh(rig)
        self.assertEqual({"FINISHED"}, bpy.ops.boneweaver.check_and_preview())
        runtime = bpy.context.window_manager.boneweaver_runtime
        self.assertTrue(PreviewController.is_enabled())
        from boneweaver.ui import draw
        cache_count = len(draw.preview_cache())
        bpy.context.scene.boneweaver_settings.preview_show_virtual_tips = False
        self.assertEqual("ANALYZED", runtime.state)
        self.assertTrue(PreviewController.is_enabled())
        self.assertLessEqual(len(draw.preview_cache()), cache_count)
        bpy.context.scene.boneweaver_settings.preview_axis_scale *= 1.1
        self.assertEqual("ANALYZED", runtime.state)
        self.assertTrue(PreviewController.is_enabled())
        bpy.context.scene.boneweaver_settings.minimum_weight += 0.01
        self.assertEqual("STALE", runtime.state)
        self.assertFalse(PreviewController.is_enabled())
        self.assertEqual("BONEWEAVER_SETTINGS_CHANGED_AFTER_ANALYZE", runtime.last_error)
    ...
    def test_changed_selection_blocks_poll_and_direct_apply(self):
        ...
        self.assertEqual({"FINISHED"}, bpy.ops.boneweaver.analyze())
        rig.pose.bones[2].select = False
        self.assertFalse(bpy.ops.boneweaver.apply.poll())
        ...
        self.assertEqual("STALE", bpy.context.window_manager.boneweaver_runtime.state)
