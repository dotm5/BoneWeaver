...
from boneweaver.core.validation import capture_neutral_meshes, validate_post_apply
...
class ValidationTests(unittest.TestCase):
    ...
    def test_post_validation_checks_graph_digests_and_neutral_mesh(self) -> None:
        ...
        bpy.ops.boneweaver.apply(plan_id=runtime.plan_id)
    ...
    def test_validate_and_export_diagnostic_json(self) -> None:
        ...
        runtime = bpy.context.window_manager.boneweaver_runtime
        bpy.ops.boneweaver.apply(plan_id=runtime.plan_id)
        self.assertNotIn("validation_scope", bpy.types.BONEWEAVER_OT_validate.bl_rna.properties)
        self.assertEqual(bpy.ops.boneweaver.validate(), {"FINISHED"})
        output = Path(__file__).resolve().parents[1] / "test-output" / "diagnostic-report.json"
        output.parent.mkdir(parents=True, exist_ok=True)
        self.assertEqual(bpy.ops.boneweaver.export_report(filepath=str(output)), {"FINISHED"})
        payload = json.loads(output.read_text(encoding="utf-8"))
        self.assertEqual(payload["kind"], "boneweaver.diagnostic_report")
