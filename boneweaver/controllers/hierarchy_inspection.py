...
class HierarchyInspectionController:
    ...
    def _mark_analyzed_plan_stale(context) -> None:
        ...
        runtime.last_error = "BONEWEAVER_HIERARCHY_SCOPE_CHANGED_REANALYZE"
    ...
    def _active_armature(context):
        ...
        if armature is None:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_NO_ACTIVE_ARMATURE")
        return armature
    ...
    def _active_bone_name(context, armature) -> str:
        ...
        if active is None or active.name not in armature.data.bones:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_HIERARCHY_NO_ACTIVE_BONE")
        return active.name
    ...
    def _validated_session(cls, context) -> tuple[HierarchyInspectionSession, object]:
        ...
        if session is None:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_HIERARCHY_INSPECTION_MISSING")
        if cls._source_filepath() != session.source_filepath:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_HIERARCHY_FILE_CHANGED")
    ...
    def use_scope(cls, context) -> FrozenInspectionScope:
        ...
        if get_used_semantic_scope() is not None:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_SCOPE_SOURCE_CONFLICT")
        ...
        if not scope.bone_names:
            raise HierarchyInspectionRuntimeError("BONEWEAVER_EMPTY_SELECTION")
    ...
    def set_branch_continuation(cls, context, branch_name: str, child_name: str):
        ...
        if (
            ...
            raise HierarchyInspectionRuntimeError("BONEWEAVER_HIERARCHY_BRANCH_INVALID")
