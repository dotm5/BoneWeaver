...
def _algorithm_setting_changed(_settings, context) -> None:
    runtime = getattr(getattr(context, "window_manager", None), "boneweaver_runtime", None)
...
class BONEWEAVER_PG_TerminalOverride(bpy.types.PropertyGroup):
...
class BONEWEAVER_PG_BranchOverride(bpy.types.PropertyGroup):
    armature_data_name: StringProperty(default="")
    armature_structural_fingerprint: StringProperty(default="")
...
class BONEWEAVER_PG_Settings(bpy.types.PropertyGroup):
    ...
    terminal_overrides: CollectionProperty(type=BONEWEAVER_PG_TerminalOverride)
    branch_overrides: CollectionProperty(type=BONEWEAVER_PG_BranchOverride)


class BONEWEAVER_PG_Runtime(bpy.types.PropertyGroup):
    ...
    quick_processed_bones: IntProperty(default=0, min=0)
    quick_component_count: IntProperty(default=0, min=0)
    quick_connected_edges: IntProperty(default=0, min=0)
...
class BONEWEAVER_PG_ChainItem(bpy.types.PropertyGroup):
...
class BONEWEAVER_PG_BoneProposalItem(bpy.types.PropertyGroup):
    bone_name: StringProperty()
    role: StringProperty()
    confidence: FloatProperty(default=0.0, min=0.0, max=1.0)


class BONEWEAVER_PG_IssueItem(bpy.types.PropertyGroup):
...
class BONEWEAVER_PG_SemanticChainItem(bpy.types.PropertyGroup):
    chain_id: StringProperty()
    root_name: StringProperty()
...
PROPERTY_CLASSES = (
    ...
    BONEWEAVER_PG_BranchOverride,
    BONEWEAVER_PG_Settings,
    BONEWEAVER_PG_Runtime,
    BONEWEAVER_PG_ChainItem,
    BONEWEAVER_PG_BoneProposalItem,
    BONEWEAVER_PG_IssueItem,
    BONEWEAVER_PG_SemanticChainItem,
)


def register_properties() -> None:
    bpy.types.Scene.boneweaver_settings = PointerProperty(type=BONEWEAVER_PG_Settings)
    bpy.types.WindowManager.boneweaver_runtime = PointerProperty(type=BONEWEAVER_PG_Runtime)
    bpy.types.WindowManager.boneweaver_chain_items = CollectionProperty(type=BONEWEAVER_PG_ChainItem)
    bpy.types.WindowManager.boneweaver_proposal_items = CollectionProperty(type=BONEWEAVER_PG_BoneProposalItem)
    bpy.types.WindowManager.boneweaver_issue_items = CollectionProperty(type=BONEWEAVER_PG_IssueItem)
    bpy.types.WindowManager.boneweaver_semantic_chain_items = CollectionProperty(
        type=BONEWEAVER_PG_SemanticChainItem,
    )
    ...
    for window_manager in getattr(bpy.data, "window_managers", ()):
        runtime = window_manager.boneweaver_runtime
        ...
        if runtime.quick_snapshot_text_name:
...
def unregister_properties() -> None:
    for name in (
        "boneweaver_semantic_chain_items",
        "boneweaver_issue_items",
        "boneweaver_proposal_items",
        "boneweaver_chain_items",
    ):
        if hasattr(bpy.types.WindowManager, name):
            delattr(bpy.types.WindowManager, name)
    if hasattr(bpy.types.WindowManager, "boneweaver_runtime"):
        del bpy.types.WindowManager.boneweaver_runtime
    if hasattr(bpy.types.Scene, "boneweaver_settings"):
        del bpy.types.Scene.boneweaver_settings
