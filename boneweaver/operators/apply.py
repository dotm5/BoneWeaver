...
class BONEWEAVER_OT_apply(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        count = context.window_manager.boneweaver_runtime.plan_bone_count
        message = f"将调整 {count} 根骨骼的方向与连接方式；不会修改网格、权重、骨名或父子层级。"
        try:
            return context.window_manager.invoke_confirm(self, event, title="应用 BoneWeaver 转换", message=message)
        except TypeError:
