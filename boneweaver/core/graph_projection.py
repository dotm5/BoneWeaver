...
def build_proposals(
    ...
    *,
    ...
    terminal_by_bone = {
        solution.bone_name: solution for solution in terminal_solutions
    }
    ...
    for edge in graph.edges:
        ...
        if parent.kind != "REAL_BONE" or not parent.mutation_target:
            continue
        tip_helper_child = (
            child.kind == "REAL_BONE"
            and child.semantic_role == "EXISTING_TIP_HELPER"
        )
        reference_only_tip = tip_helper_child and child.reference_only
        terminal_solution = terminal_by_bone.get(parent.bone_name)
        manual_terminal = (
            reference_only_tip
            and terminal_solution is not None
            and terminal_solution.source == "MANUAL_OVERRIDE"
        )
        ...
        if profile == "VISUAL_CHAIN_CLEANUP":
            connect = (
                (parent.bone_name not in root_bones or parent.bone_name in main_children)
                and parent.bone_name not in side_children
            )
        elif profile in {"BONEX_ROTATION_CHAIN", "WIGGLE2_ROTATION_CHAIN"}:
        ...
        proposals.append(
            BoneProposal(
                ...
                state.roll,
                terminal_solution.tail if manual_terminal else child.joint_position,
                state.local_z,
                connect,
                (
                    terminal_solution.source if manual_terminal
                    else child.source if edge.kind == "VIRTUAL_TIP_SEGMENT" or tip_helper_child
                    else "UNIQUE_DIRECT_CHILD_HEAD"
                ),
                (
                    resolution.score if resolution is not None
                    else terminal_solution.confidence if manual_terminal
                    else 1.0
                ),
                (),
            )
