...
def run_preflight(context, *, scope_names: tuple[str, ...] | None = None) -> PreflightResult:
    ...
    if armature is None:
        return PreflightResult(None, None, (), (), (), (_issue("BLOCKER", "BONEWEAVER_NO_ACTIVE_ARMATURE", "No active Armature"),))

    if armature.library or armature.data.library:
        issues.append(_issue("BLOCKER", "BONEWEAVER_LINKED_ARMATURE", "Armature is linked and not editable", objects=(armature.name,)))
    if armature.data.users > 1:
        issues.append(_issue("BLOCKER", "BONEWEAVER_SHARED_ARMATURE_DATA", "Armature data has multiple object users", objects=(armature.name,)))

    determinant = armature.matrix_world.to_3x3().determinant()
    if not math.isfinite(determinant) or abs(determinant) <= 1.0e-12:
        issues.append(_issue("BLOCKER", "BONEWEAVER_NON_INVERTIBLE_TRANSFORM", "Armature transform is not invertible", objects=(armature.name,)))
    elif determinant < 0.0:
        issues.append(_issue("BLOCKER", "BONEWEAVER_NEGATIVE_OBJECT_TRANSFORM", "Armature transform has negative determinant", objects=(armature.name,)))
    ...
    if not selected:
        issues.append(_issue("BLOCKER", "BONEWEAVER_EMPTY_SELECTION", "No target bones are selected", objects=(armature.name,)))
    ...
    if selected:
        try:
            ...
            issues.append(_issue("BLOCKER", "BONEWEAVER_UNSUPPORTED_CONTEXT", str(error), objects=(armature.name,)))

    selected_set = set(selected)
    for name in selected:
        bone = armature.data.bones[name]
        if bone.bbone_segments > 1:
            issues.append(_issue("BLOCKER", "BONEWEAVER_BBONE_UNSUPPORTED", "B-Bone segments are unsupported", bones=(name,)))
        for child in bone.children:
            if child.name not in selected_set and child.use_connect:
                issues.append(_issue("BLOCKER", "BONEWEAVER_EXTERNAL_CONNECTED_CHILD", "Unselected connected child would be moved", bones=(name, child.name)))
            if child.name in selected_set and (child.head_local - bone.head_local).length <= 1.0e-7:
                issues.append(_issue("BLOCKER", "BONEWEAVER_COINCIDENT_HELPER", "Parent and child heads coincide", bones=(name, child.name)))
        pose_bone = armature.pose.bones.get(name)
        if pose_bone:
            if pose_bone.constraints:
                issues.append(_issue("BLOCKER", "BONEWEAVER_RELATED_CONSTRAINT", "Target bone has constraints", bones=(name,)))
    ...
    if non_identity:
        issues.append(_issue("BLOCKER", "BONEWEAVER_NON_IDENTITY_POSE", "Armature pose is not identity", bones=non_identity))

    animation = armature.animation_data
    if animation:
        if animation.action:
            issues.append(_issue("BLOCKER", "BONEWEAVER_RELATED_ACTION", "Armature has an active Action", objects=(armature.name,)))
        if animation.nla_tracks:
            issues.append(_issue("BLOCKER", "BONEWEAVER_RELATED_NLA", "Armature has NLA tracks", objects=(armature.name,)))
        if animation.drivers:
            issues.append(_issue("BLOCKER", "BONEWEAVER_RELATED_DRIVER", "Armature has drivers", objects=(armature.name,)))

    for obj in bpy.data.objects:
        if obj.parent == armature and obj.parent_type == "BONE" and obj.parent_bone in selected_set:
            issues.append(_issue("BLOCKER", "BONEWEAVER_BONE_PARENTED_OBJECT", "Object is parented to a target bone", bones=(obj.parent_bone,), objects=(obj.name,)))
        for constraint in obj.constraints:
            if getattr(constraint, "target", None) == armature and getattr(constraint, "subtarget", "") in selected_set:
                issues.append(_issue("BLOCKER", "BONEWEAVER_RELATED_CONSTRAINT", "Object constraint targets a selected bone", objects=(obj.name,)))
    ...
    if not bindings:
        issues.append(_issue("BLOCKER", "BONEWEAVER_NO_ASSOCIATED_MESH", "No associated Mesh was found", objects=(armature.name,)))
    for binding in bindings:
        mesh = bpy.data.objects[binding.object_name]
        modifier = mesh.modifiers[binding.modifier_name]
        if modifier.use_bone_envelopes:
            issues.append(_issue("BLOCKER", "BONEWEAVER_ENVELOPE_DEFORMATION", "Armature modifier uses bone envelopes", objects=(mesh.name,)))
        for prior in mesh.modifiers[: mesh.modifiers.find(modifier.name)]:
            if prior.type in {"MIRROR", "ARRAY", "SUBSURF", "REMESH", "NODES", "BOOLEAN", "SKIN"}:
                issues.append(_issue("BLOCKER", "BONEWEAVER_TOPOLOGY_MODIFIER_BEFORE_ARMATURE", "Topology modifier precedes Armature modifier", objects=(mesh.name,)))
                break
