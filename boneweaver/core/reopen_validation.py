...
def validate_reopened_file():
    manifest = _json_text("BONEWEAVER_EXPORT_MANIFEST")
    if manifest is None:
        return _empty_report("BONEWEAVER_REOPEN_MANIFEST_MISSING")
    snapshot_id = manifest.get("snapshot_id")
    snapshot = _json_text(f"BONEWEAVER_SNAPSHOT::{snapshot_id}") if snapshot_id else None
    if snapshot is None:
        return _empty_report("BONEWEAVER_REOPEN_SNAPSHOT_MISSING")
    issues = set()
    if snapshot.get("status") != "APPLIED":
        issues.add("BONEWEAVER_REOPEN_SNAPSHOT_NOT_APPLIED")
    ...
    if (
        ...
        issues.add("BONEWEAVER_REOPEN_TIP_HELPER_CLASSIFICATION_MISMATCH")
    ...
    if manifest_records != snapshot_records:
        issues.add("BONEWEAVER_REOPEN_MUTATION_LEDGER_MISMATCH")
    ...
    if mutation_targets != tuple(snapshot.get("mutation_targets", ())):
        issues.add("BONEWEAVER_REOPEN_TOPOLOGY_LEDGER_MISMATCH")
    ...
    if reference_only_helper_set.intersection(mutation_targets) or any(
        ...
        issues.add("BONEWEAVER_REOPEN_TIP_HELPER_MUTATION_DETECTED")
    for helper in manifest_helpers:
    ...
    if armature is None or armature.type != "ARMATURE" or armature.data.name != armature_info.get("data_name"):
        return _empty_report("BONEWEAVER_REOPEN_ARMATURE_MISSING")
    ...
    if not _vector_close(armature_matrix, armature_info.get("matrix_world", ())):
        issues.add("BONEWEAVER_REOPEN_REST_GEOMETRY_MISMATCH")
    ...
    for name, expected in expected_post.items():
        ...
        if actual is None:
            issues.add("BONEWEAVER_REOPEN_BONE_MISSING")
            continue
        ...
        if actual["use_connect"] != bool(expected.get("use_connect")):
            issues.add("BONEWEAVER_REOPEN_CONNECT_MISMATCH")
    ...
    for helper in manifest_helpers:
        ...
        if geometry_mismatch:
            issues.add("BONEWEAVER_REOPEN_TIP_HELPER_GEOMETRY_MISMATCH")
    ...
    for branch, selected_child in selected_by_branch.items():
        ...
        if not branch_state or not child_state or not _vector_close(branch_state["tail"], child_state["head"]):
            issues.add("BONEWEAVER_REOPEN_BRANCH_MAIN_PATH_MISMATCH")
        ...
        if any(item["use_connect"] for item in side_states.values()):
            issues.add("BONEWEAVER_REOPEN_BRANCH_SIDE_CONNECT_MISMATCH")
    ...
    for edge in manifest.get("physics_edges", ()):
        ...
        if parent in states and child in states:
            ...
            if not _vector_close(states[parent]["tail"], states[child]["head"]):
                issues.add("BONEWEAVER_REOPEN_LINEAR_CONTINUITY_MISMATCH")
    ...
    for name, expected_weight in manifest.get("weight_digests", {}).items():
        ...
        if modifier_digest(mesh) != manifest.get("modifier_digests", {}).get(name):
            modifier_changes += 1
    if weight_changes:
        issues.add("BONEWEAVER_WEIGHT_DIGEST_CHANGED")
    if base_changes:
        issues.add("BONEWEAVER_BASE_MESH_CHANGED")
    if modifier_changes:
        issues.add("BONEWEAVER_MODIFIER_DIGEST_CHANGED")
    ...
    if packed_count != expected_packed:
        issues.add("BONEWEAVER_REOPEN_PACKED_IMAGE_COUNT_MISMATCH")
    return ReopenValidationReport(
        success=not issues,
