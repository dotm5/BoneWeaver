...
def branch_auto_eligibility_issue_codes(branch_bone_name, child_names, by_name):
    ...
    for state in states:
        ...
        if state.is_socket or tokens.intersection(_SECONDARY_RULES.socket_tokens) or "SOCKET" in flags:
            issue_codes.add("BONEWEAVER_BRANCH_AUTO_SOCKET_FORBIDDEN")
        if tokens.intersection(_SECONDARY_RULES.ik_control_tokens) or flags.intersection({"IK", "CONTROL"}):
            issue_codes.add("BONEWEAVER_BRANCH_AUTO_CONTROL_FORBIDDEN")
        if tokens.intersection(_SECONDARY_RULES.twist_deform_tokens):
            issue_codes.add("BONEWEAVER_BRANCH_AUTO_TWIST_FORBIDDEN")
        if (
            ...
            issue_codes.add("BONEWEAVER_BRANCH_AUTO_MAIN_SKELETON_FORBIDDEN")
        if tokens.intersection(_SECONDARY_RULES.facial_tokens) or not state.use_deform:
            issue_codes.add("BONEWEAVER_BRANCH_AUTO_SECONDARY_SEMANTICS_REQUIRED")
...
def resolve_branch(
    ...
    *,
    ...
    if mode == "MANUAL_ONLY":
        ...
        return BranchResolution(branch_bone_name, mode, ranked, None, child_names, 0.0, 0.0, "AMBIGUOUS", True, ("BONEWEAVER_BRANCH_AMBIGUOUS",))
    winner = ranked[0]
    runner_up = ranked[1]
    ...
    if winner.score >= defaults.high_score and margin >= defaults.high_margin:
        ...
        return BranchResolution(branch_bone_name, mode, ranked, None, child_names, winner.score, margin, "AMBIGUOUS", True, ("BONEWEAVER_BRANCH_AMBIGUOUS",))
    return BranchResolution(
