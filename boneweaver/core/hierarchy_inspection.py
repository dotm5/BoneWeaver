...
def _walk_linear(
    ...
    while True:
        ...
        if len(children) > 1:
            ...
            accumulator.issues.add("BONEWEAVER_HIERARCHY_BRANCH_AMBIGUOUS")
            return
        current = children[0]
...
def _walk_same_stem(
    ...
    while True:
        ...
        if len(matching) > 1:
            accumulator.side_branch_roots.update(children)
            accumulator.issues.add("BONEWEAVER_HIERARCHY_STEM_AMBIGUOUS")
            return
        if not matching:
            if len(children) > 1:
                accumulator.side_branch_roots.update(children)
            accumulator.issues.add("BONEWEAVER_HIERARCHY_STEM_NO_CONTINUATION")
            return
    ...
    while True:
        ...
        if continuation not in children:
            ...
            accumulator.issues.add(
                "BONEWEAVER_HIERARCHY_CONTINUATION_INVALID"
                if continuation is not None
                else "BONEWEAVER_HIERARCHY_BRANCH_AMBIGUOUS"
            )
            return
        accumulator.side_branch_roots.update(name for name in children if name != continuation)
        current = continuation
...
def build_hierarchy_inspection_plan(
    ...
    if missing_selected:
        accumulator.issues.add("BONEWEAVER_HIERARCHY_SELECTED_BONE_MISSING")
