...
def _mesh_resolution(
    ...
    if policy == WeightIslandPolicy.DOMINANT_COMPONENT.value:
        ...
        if not compatible:
            warnings.add("BONEWEAVER_WEIGHT_DIRECTION_CONFLICT")
    if len(components) > 1 and not selected:
        warnings.add("BONEWEAVER_DISCONNECTED_WEIGHT_ISLANDS")
        warnings.add("BONEWEAVER_WEIGHT_ISLAND_POLICY_BLOCKED")
