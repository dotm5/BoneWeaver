    ...
    def export_conversion(context, filepath: str) -> set[str]:
        ...
        try:
            ...
            if not source.is_file():
                raise RuntimeError("UECP_EXPORT_SOURCE_FILE_MISSING")
            if source.resolve() == destination.resolve():
                raise RuntimeError("UECP_EXPORT_SOURCE_OVERWRITE_FORBIDDEN")
            ...
            manifest = build_export_manifest(
                ...
                tolerance_mode=context.scene.uecp_settings.validation_tolerance_mode,
                packed_image_count=packed_count,
            )
            text = bpy.data.texts.get("UECP_EXPORT_MANIFEST") or bpy.data.texts.new("UECP_EXPORT_MANIFEST")
            text.clear()
            ...
            if file_signature(source) != source_signature:
                raise RuntimeError("UECP_EXPORT_SOURCE_FILE_CHANGED")
            ...
            if manifest["reopen_validation"]["result"] != "PASS":
                raise RuntimeError("UECP_REOPEN_VALIDATION_FAILED: " + ", ".join(manifest["reopen_validation"]["issues"])
