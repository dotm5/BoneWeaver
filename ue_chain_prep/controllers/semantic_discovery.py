...
class SemanticDiscoveryController:
    ...
    def _active_armature(context):
        ...
        if armature is None:
            raise SemanticDiscoveryRuntimeError("UECP_NO_ACTIVE_ARMATURE")
        return armature
    ...
    def _sync_runtime(context, session: SemanticDiscoverySession) -> None:
        ...
        runtime = getattr(wm, "uecp_runtime", None)
        items = getattr(wm, "uecp_semantic_chain_items", None)
        ...
        for property_name in (
            ...
            try:
                ...
                raise SemanticDiscoveryRuntimeError(
                    f"UECP_SEMANTIC_RULE_LOAD_FAILED: {property_name}: {exc}"
                ) from exc
    ...
    def _validated_session(cls, context):
        ...
        if session is None:
            raise SemanticDiscoveryRuntimeError("UECP_SEMANTIC_DISCOVERY_MISSING")
        if cls._source_filepath() != session.source_filepath:
            raise SemanticDiscoveryRuntimeError("UECP_SEMANTIC_FILE_CHANGED")
        ...
        if (
            ...
            raise SemanticDiscoveryRuntimeError("UECP_SEMANTIC_ARMATURE_CHANGED")
    ...
    def _chain(session, chain_id: str):
        ...
        if chain is None or chain.discovery_class == SemanticDiscoveryClass.EXCLUDE.value:
            raise SemanticDiscoveryRuntimeError("UECP_SEMANTIC_CHAIN_INVALID")
        return chain
    ...
    def _mark_analyzed_plan_stale(context) -> None:
        ...
        runtime.last_error = "UECP_SEMANTIC_SCOPE_CHANGED_REANALYZE"
    ...
    def use_confirmed_chains(cls, context) -> FrozenSemanticScope:
        ...
        if not session.confirmed_chain_ids:
            raise SemanticDiscoveryRuntimeError("UECP_SEMANTIC_CONFIRMATION_REQUIRED")
        ...
        if not names:
            raise SemanticDiscoveryRuntimeError("UECP_EMPTY_SELECTION")
