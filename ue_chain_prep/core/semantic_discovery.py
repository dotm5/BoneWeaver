...
_EXCLUSION_REASON_BY_CATEGORY = {
    SecondaryBoneCategory.MAIN_SKELETON.value: "UECP_SEMANTIC_EXCLUDE_MAIN_SKELETON",
    SecondaryBoneCategory.SOCKET.value: "UECP_SEMANTIC_EXCLUDE_SOCKET",
    SecondaryBoneCategory.IK_CONTROL.value: "UECP_SEMANTIC_EXCLUDE_IK_CONTROL",
    SecondaryBoneCategory.TWIST_DEFORM.value: "UECP_SEMANTIC_EXCLUDE_TWIST_DEFORM",
    SecondaryBoneCategory.FACIAL.value: "UECP_SEMANTIC_EXCLUDE_FACIAL",
}
...
def _category_decision(state, rules, metadata, main_patterns):
    ...
    if strong:
        semantic_score = 1.0
        reasons.append("UECP_SEMANTIC_STRONG_INCLUDE_TOKEN")
    elif medium:
        semantic_score = 0.65
        reasons.append("UECP_SEMANTIC_MEDIUM_INCLUDE_TOKEN")
    elif generic:
        semantic_score = 0.25
        reasons.append("UECP_SEMANTIC_GENERIC_INCLUDE_TOKEN")
    else:
        semantic_score = 0.0
...
def _weight_score(summary):
    if summary is None:
        return 0.0, ("UECP_SEMANTIC_WEIGHT_EVIDENCE_UNAVAILABLE",)
    ...
    if (
        ...
        or total_weight <= 0.0
        or "UECP_INSUFFICIENT_WEIGHT_CLOUD" in warnings
    ):
        return 0.0, ("UECP_SEMANTIC_NO_WEIGHT_SUPPORT",)
    sample_support = min(1.0, sample_count / 8.0)
    score = max(confidence, 0.70 * sample_support)
    return score, ("UECP_SEMANTIC_WEIGHT_SUPPORT",)
...
def _build_bone_evidence(states, rules, weight_by_bone, metadata_by_bone):
    ...
    for state in states:
        ...
        if compatible:
            reasons.add("UECP_SEMANTIC_HIERARCHY_CONTINUITY")
        consecutive = any(
        ...
        if consecutive:
            sequence_score = 1.0
            reasons.add("UECP_SEMANTIC_SEQUENCE_CONTINUITY")
        elif missing_sequence:
            sequence_score = 0.65
            reasons.add("UECP_SEMANTIC_SEQUENCE_IMPLICIT")
        elif compatible:
            sequence_score = 0.35
            reasons.add("UECP_SEMANTIC_SEQUENCE_GAP")
        else:
        ...
        if (
            ...
            reasons.add("UECP_SEMANTIC_NO_SECONDARY_EVIDENCE")
        evidence.append(
            SemanticBoneEvidence(
...
def _chain_from_component(component, states_by_name, evidence_by_name, armature_fingerprint, rule_set_ids):
    ...
    reasons.add("UECP_SEMANTIC_CHAIN_GROUPED")
    if branch_names:
        reasons.add("UECP_SEMANTIC_BRANCH_REVIEW_REQUIRED")
        discovery_class = _cap_class(
            discovery_class, SemanticDiscoveryClass.SUGGEST_INCLUDE.value,
        )
    if len(categories) > 1 or len(roots) > 1:
        reasons.add("UECP_SEMANTIC_CHAIN_CATEGORY_CONFLICT")
        discovery_class = SemanticDiscoveryClass.AMBIGUOUS.value
