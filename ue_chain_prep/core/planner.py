...
_BRANCH_BLOCK_MESSAGES = {
    "UECP_BRANCH_AMBIGUOUS": "Branch continuation evidence is ambiguous",
    "UECP_BRANCH_AUTO_MAIN_SKELETON_FORBIDDEN": "Automatic branch selection is forbidden on main-skeleton semantics",
    "UECP_BRANCH_AUTO_CONTROL_FORBIDDEN": "Automatic branch selection is forbidden on control or IK semantics",
    "UECP_BRANCH_AUTO_SOCKET_FORBIDDEN": "Automatic branch selection is forbidden on socket semantics",
    "UECP_BRANCH_AUTO_TWIST_FORBIDDEN": "Automatic branch selection is forbidden on twist semantics",
    "UECP_BRANCH_AUTO_SECONDARY_SEMANTICS_REQUIRED": "Automatic branch selection requires explicit secondary-physics semantics",
}
...
def build_plan(
    ...
    *,
    ...
    tip_usage_issues = [
        ValidationIssue(
            "BLOCKER", "UECP_HIERARCHY_TIP_HELPER_IDENTITY_CHANGED",
    ...
    if tip_helper_usage == TipHelperUsage.INCLUDE_AS_PHYSICS_TERMINAL.value:
        if not visual_cleanup:
            tip_usage_issues.append(
                ValidationIssue(
                    "BLOCKER", "UECP_TIP_HELPER_INCLUDE_PROFILE_REQUIRED",
            ...
            for classification in tip_helpers:
                if classification.excluded_child_names:
                    tip_usage_issues.append(
                        ValidationIssue(
                            "BLOCKER", "UECP_TIP_HELPER_INCLUDE_UNSAFE",
    ...
    effective_preflight_issues = tuple(
        ...
        if not (
            issue.code == "UECP_EXTERNAL_CONNECTED_CHILD"
    ...
    for classification in tip_helpers:
        ...
        if manual is not None and manual.requires_confirmation:
            override_issues.append(
                ValidationIssue(
                    "BLOCKER", "UECP_VIRTUAL_TIP_INVALID",
        ...
        if (
            ...
            override_issues.append(
                ValidationIssue(
                    "BLOCKER", "UECP_MANUAL_OVERRIDE_CONNECTED_TIP_HELPER",
